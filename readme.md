...
- `deepness of uses`: an array with a size equal to the number of uses of the virtual register. The values in the array refer to the depth in loop structures of each variable usage. For example, if a value is not inside any loop, its depth value will be zero. If it is inside one loop, its depth will be one, if inside two nested loops, it will be two, and so on;
